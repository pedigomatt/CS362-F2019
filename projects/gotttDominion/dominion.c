...
int minionCardEffect( int choice1 , int choice2 , int handPos , struct gameState* state, int currentPlayer )
{
	...
	//+1 action
	state->numActions++;
    printf("Pre Discard Card Count: %d\n", state->discardCount[currentPlayer]);

	//discard card from hand
	discardCard(handPos, currentPlayer, state, 1); 
    printf("Post Discard Card Count: %d\n", state->discardCount[currentPlayer]);
	...
	else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
	{
		...
		{
            printf("Draw Card Number: %d\n", i);
			drawCard(currentPlayer, state);
            printf("Player handcount number: %d\n", state->handCount[currentPlayer]);
		}
...
int ambassadorCardEffect(int choice1, int choice2, int handPos, struct gameState* state, int currentPlayer)
{
	...
	for (i = 0; i < 2 ; i++) 
	{
        printf("Player Number gaining Card: %d\n", i);

		nif (i != currentPlayer)
		{
            printf("Player Number gaining Card not current Player: %d\n", i);
	...
	//discard played card from hand
    printf("Pre Discard Card Count: %d\n", state->discardCount[currentPlayer]);
    printf("discardCard inputs: player: %d, state: %d, handpos: %d, trigger: 0\n", currentPlayer, state, handpos);
